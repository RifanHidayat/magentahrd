...
class _CheckinPageState extends State<CheckinPage> {
                ...
                child: Container(
                  child: Column(
                    children: <Widget>[
                      ...
                      Container(
                        child: _is_aerplus_employee == true &&
                                _active_working_pattern_id.toString().isEmpty
                            ? Column(
                                ...
                                children: <Widget>[
                                  ...
                                  for (var workingPattern in _working_patterns!)
                                    ListTile(
                                      title: Text(
                                          workingPattern['time_description']),
                                      leading: Container(
                                        margin: EdgeInsets.only(left: 10),
                                        child: Radio<int>(
                                          value: workingPattern['id'],
                                          groupValue: workingPatternId,
                                          onChanged: (value) {
                                            setState(() {
  ...
  Future upload() async {
    ...
    var activeWorkingPatternId = _active_working_pattern_id;

    if (_active_working_pattern_id.toString().isEmpty ||
        _active_working_pattern_id == null ||
        _active_working_pattern_id == "") {
      if (_is_aerplus_employee || _is_aerplus_employee == "true") {
        activeWorkingPatternId = workingPatternId;
      }
    }
    ...
    if (_category_absent.toString().toLowerCase() != 'present') {
      ...
      if (_active_working_pattern_id.toString().isEmpty ||
          ...
          _active_working_pattern_id == "") {
        if (_is_aerplus_employee || _is_aerplus_employee == "true") {
          Toast.show("Shift tidak boleh kosong", context,
              duration: 5, gravity: Toast.BOTTOM);
        }
      } else {
  ...
  Future dataEmployee(var id) async {
    try {
      ...
      setState(() {
        ...
        _is_aerplus_employee = _employee!['data']['is_aerplus_employee'];
  ...
  Future dataWorkingPattern() async {
    try {
      setState(() {
        _isLoading = true;
      });
      final response = await http
          .get(Uri.parse("$base_url/api/working-patterns?division=aerplus"));
      final data = jsonDecode(response.body);
      setState(() {
        _working_patterns = data!['data'];
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
